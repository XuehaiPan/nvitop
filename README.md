...
*Note: `nvitop` uses only one character to indicate the type of processes. `C` stands for compute processes, `G` for graphic processes, and `X` for both (i.e. MI(X), in `nvidia-smi` it is `C+G`).*
